...
//!
//! Direct balance querying, prompt cache metrics extraction, and capability probing for New-API.
    ...
    async fn fetch_usage(
        ...
        let records = items.into_iter().map(|item| {
            ...
            let cache_read_tokens = item.get("cache_read_tokens")
                .or_else(|| item.get("cache_read"))
                .or_else(|| item.get("cached_tokens"))
                .or_else(|| item.get("cache_read_input_tokens"))
                .and_then(|v| v.as_u64())
                .unwrap_or(0);

            let cache_write_tokens = item.get("cache_write_tokens")
                .or_else(|| item.get("cache_write"))
                .or_else(|| item.get("cache_creation_input_tokens"))
                .and_then(|v| v.as_u64())
                .unwrap_or(0);
