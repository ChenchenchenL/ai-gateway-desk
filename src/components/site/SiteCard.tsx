...
/**
 * Windows 11 Fluent Acrylic Summary Card for a monitored gateway site.
 */
export const SiteCard: React.FC<SiteCardProps> = ({ site, onSelect }) => {
  ...
  return (
    <div
      ...
      className="fluent-card p-3.5 rounded-2xl cursor-pointer transition flex flex-col gap-2 relative overflow-hidden group select-none"
    >
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] px-2 py-0.5 rounded-lg bg-white/[0.06] text-slate-300 font-mono uppercase border border-white/10 font-semibold">
          {site.provider}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between pt-0.5">
        ...
        {site.capabilities?.window_quota && (
          <div className="flex flex-col items-end">
            ...
            <span className="text-xs font-semibold text-blue-300 tabular-digits">
      ...
      <div className="flex items-center justify-between text-[10px] text-slate-400 pt-1.5 border-t border-white/[0.06]">
        <div className="flex items-center gap-1 truncate max-w-[190px]">
          {isOutdated ? (
            ...
            <span className="text-rose-400/90 truncate font-medium">{site.last_error}</span>
          ) : (
